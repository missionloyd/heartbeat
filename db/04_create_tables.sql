...
CREATE TABLE IF NOT EXISTS public.measurement_prediction_type(
  id INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  name VARCHAR(255) NOT NULL UNIQUE
);
...
CREATE TABLE IF NOT EXISTS public.measurement(
  ...
  measurement_prediction_type_id INTEGER NOT NULL,
  ts timestamp without time zone default (now() at time zone 'utc'),
  value NUMERIC NOT NULL,
  UNIQUE(asset_id, measurement_type_id, ts, measurement_prediction_type_id),
  FOREIGN KEY(asset_id) REFERENCES asset(id),
  FOREIGN KEY(measurement_type_id) REFERENCES measurement_type(id),
  FOREIGN KEY(measurement_prediction_type_id) REFERENCES measurement_prediction_type(id)
);
